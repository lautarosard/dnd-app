    ...
    useEffect(() => {
        const invocarMagia = async () => {
            try {
                ...
                const respuesta = await fetch('https://www.dnd5eapi.co/api/2014/spells', requestOptions);
                if (!respuesta.ok) {
                    throw new Error('Error en Servidor API (error al traer datos)');
                }
                const datos = await respuesta.json();
                setHechizos(datos.results);
            } catch(err){
  ...
  return (
    <main>
      ...
      <div id="seccion-hechizos">
        <SpellDashboards listaBase={hechizos} />
      </div>      
    </main>
  );
}
